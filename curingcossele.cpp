...
#include "ui_curingcossele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringCosSele::CuringCosSele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringCosSele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("筛选固化剂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("筛选固化剂消耗标准信息"));


    ui->SeleBtn->setGeometry(370,240,75,30);
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringCosSele::close);//这个牛逼
    ...
    setTabOrder(ui->CurAgeID,ui->BathTubID);
    setTabOrder(ui->BathTubID,ui->First);
...
void CuringCosSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringCosSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringCosSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringCosSele::SeleBtnC()
{
    QStringList str;
    str<<ui->BathTubID->text()<<ui->CurAgeID->text()<<ui->First->text()<<ui->Second->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("BathID = '%1' ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("CurAgeID = '%1' ").arg(str.at(1));
    if(str.at(2) != "") data<<QString("First = %1 ").arg(str.at(2));
    if(str.at(3) != "") data<<QString("Second = %1 ").arg(str.at(3));
    ...
    if(data.count() == 0)
    {
        splus = "select * from curingagentcosting;";
    }else{
        splus = "select * from curingagentcosting where "+data.at(0)+splus+";";
    }
...
void CuringCosSele::on_CancleBtn_clicked()
{
    ui->CurAgeID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
