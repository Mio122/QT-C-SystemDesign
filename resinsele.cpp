    ...
    ui(new Ui::ResinSele)
{
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinSele::close);//这个牛逼
    ...
    ui->ResinID->setFocus();
    setTabOrder(ui->ResinID,ui->ResinName);
...
void ResinSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinSele::SeleBtnC()
{
    QStringList str;
    str<<ui->ResinID->text()<<ui->ResinName->text()<<ui->ResinPrice->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("ResinID = '%1' ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("ResinName = '%1' ").arg(str.at(1));
    if(str.at(2) != "") data<<QString("ResinPrice = %1 ").arg(str.at(2));
    ...
    if(data.count() == 0)
    {
        splus = "select * from resin;";
    }else{
        splus = "select * from resin where "+data.at(0)+splus+";";
    }
...
void ResinSele::on_CancleBtn_clicked()
{
    ui->ResinID->clear();
    ui->ResinName->clear();
    ui->ResinPrice->clear();
