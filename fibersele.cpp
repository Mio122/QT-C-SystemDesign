#include "fibersele.h"
#include "ui_fibersele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberSele::FiberSele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberSele)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("筛选玻纤信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberSele::close);//这个牛逼
    ...
    setTabOrder(ui->FiberGlassID,ui->FiberGlassName);
    setTabOrder(ui->FiberGlassName,ui->FiberGlassPrice);
...
void FiberSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberSele::SeleBtnC()
{
    QStringList str;
    str<<ui->FiberGlassID->text()<<ui->FiberGlassName->text()<<ui->FiberGlassPrice->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("FiberGlassID = '%1' ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("FiberGlassName = '%1' ").arg(str.at(1));
    if(str.at(2) != "") data<<QString("FiberGlassPrice = %1 ").arg(str.at(2));
    ...
    if(data.count() == 0)
    {
        splus = "select * from fiberglass;";
    }else{
        splus = "select * from fiberglass where "+data.at(0)+splus+";";
    }

    //qDebug()<<splus;
    emit sendData(splus);
    ui->FiberGlassID->clear();
    ui->FiberGlassName->clear();
    ui->FiberGlassPrice->clear();
    this->hide();

}

void FiberSele::on_CancleBtn_clicked()
{
    ui->FiberGlassID->clear();
    ui->FiberGlassName->clear();
    ui->FiberGlassPrice->clear();
