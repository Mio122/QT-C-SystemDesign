...
#include "ui_curingedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringEdit::CuringEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringEdit)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("编辑固化剂信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringEdit::close);//这个牛逼
...
void CuringEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringEdit::editData(QStringList data)
{
...
void CuringEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->CurAgeID->text()<<ui->CurAgeName->text()<<ui->CurAgePrice->text();
    QString data;
    data = QString("update curingagent set CurAgeName = '%1',CurAgePrice = %2 where CurAgeID = '%4';")\
...
void CuringEdit::on_CancleBtn_clicked()
{
    this->hide();
}
