...
#include "ui_fibercosdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberCosDele::FiberCosDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberCosDele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("删除玻纤消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("删除玻纤消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberCosDele::close);//这个牛逼
...
void FiberCosDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberCosDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberCosDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberCosDele::DeleBtnC()
{
    QString str;
    str = QString("delete from fiberglasscosting where BathID = '%1';").arg(ui->BathTubID->text());
    emit sendData(str);
    this->hide();
}

void FiberCosDele::deleData(QStringList data)
{
...
void FiberCosDele::on_CancleBtn_clicked()
{
    this->hide();
}
