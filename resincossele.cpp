...
#include "ui_resincossele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件


ResinCosSele::ResinCosSele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ResinCosSele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("筛选树脂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("筛选树脂消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinCosSele::close);//这个牛逼
    ...
    ui->ResinID->setFocus();
    setTabOrder(ui->ResinID,ui->BathTubID);
...
void ResinCosSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinCosSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinCosSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinCosSele::SeleBtnC()
{
    QStringList str;
    str<<ui->BathTubID->text()<<ui->ResinID->text()<<ui->First->text()<<ui->Second->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("BathID = '%1' ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("ResinID = '%1' ").arg(str.at(1));
    ...
    if(data.count() == 0)
    {
        splus = "select * from resincosting;";
    }else{
        splus = "select * from resincosting where "+data.at(0)+splus+";";
    }
...
void ResinCosSele::on_CancleBtn_clicked()
{
    ui->ResinID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
