    ...
    ui(new Ui::OutPut)
{
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&OutPut::close);//这个牛逼
    ...
    QSettings *config = new QSettings("D:\\Program Files (x86)\\MySQL\\MySQL Server 5.5\\my.ini", QSettings::IniFormat);

    QString address = config->value("mysqld/secure_file_priv").toString();


    ui->FileEdit->setText(address);
...
void OutPut::SaveBtnC()
{
    QString data;
    QString table;
    QString Address;
    Address = ui->FileEdit->text()+"/"+ui->FileNameEdit->text();
    table = MainWindow::Table;
    table.replace(QRegExp(";$"),"");
    data = QString("select * from (%1) as tmp into outfile '%2' fields terminated by ',' optionally enclosed by '\"' lines terminated by '\\n';")
            .arg(table).arg(Address);
...
void OutPut::on_CancleBtn_clicked()
{
    ui->FileNameEdit->clear();
    this->hide();
}
