...
#include "ui_curingcosedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringCosEdit::CuringCosEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringCosEdit)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("编辑固化剂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("编辑固化剂消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringCosEdit::close);//这个牛逼
...
void CuringCosEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringCosEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringCosEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringCosEdit::editData(QStringList data)
{
    ui->BathTubID->setText(data.at(0));
    ui->CurAgeID->setText(data.at(1));
...
void CuringCosEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->BathTubID->text()<<ui->CurAgeID->text()<<ui->First->text()<<ui->Second->text();
    QString data;
    data = QString("update curingagentcosting set CurAgeID = '%1',First = %2 ,Second = %3 where BathID = '%4';")\
...
void CuringCosEdit::on_CancleBtn_clicked()
{
    this->hide();
}
