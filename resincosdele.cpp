...
#include "ui_resincosdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

ResinCosDele::ResinCosDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ResinCosDele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("删除树脂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("删除树脂消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinCosDele::close);//这个牛逼
...
void ResinCosDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinCosDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinCosDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinCosDele::DeleBtnC()
{
    QString str;
    str = QString("delete from resincosting where BathID = '%1';").arg(ui->BathTubID->text());
    emit sendData(str);
    this->hide();

}

void ResinCosDele::deleData(QStringList data)
{
...
void ResinCosDele::on_CancleBtn_clicked()
{
    this->hide();
