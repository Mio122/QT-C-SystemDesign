...
#include "ui_fibercossele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberCosSele::FiberCosSele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberCosSele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("筛选玻纤消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("筛选玻纤消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberCosSele::close);//这个牛逼
    ...
    ui->FiberGlassID->setFocus();
    setTabOrder(ui->FiberGlassID,ui->BathTubID);
...
void FiberCosSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberCosSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberCosSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberCosSele::SeleBtnC()
{
    ...
    if(data.count() == 0)
    {
        splus = "select * from fiberglasscosting;";
    }else{
        splus = "select * from fiberglasscosting where "+data.at(0)+splus+";";
    }
...
void FiberCosSele::on_CancleBtn_clicked()
{
    ui->FiberGlassID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
