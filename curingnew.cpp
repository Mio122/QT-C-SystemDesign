#include "curingnew.h"
#include "ui_curingnew.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringNew::CuringNew(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringNew)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("新建固化剂信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringNew::close);//这个牛逼
...
void CuringNew::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringNew::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringNew::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringNew::SaveBtnC()
{
    QString str;
    str = QString("insert into curingagent values('%1','%2',%3);")\
            .arg(ui->CurAgeID->text()).arg(ui->CurAgeName->text()).arg(ui->CurAgePrice->text());
    emit sendData(str);
    ui->CurAgeID->clear();
    ui->CurAgeName->clear();
...
void CuringNew::on_CancleBtn_clicked()
{
    ui->CurAgeID->clear();
    ui->CurAgeName->clear();
    ui->CurAgePrice->clear();
