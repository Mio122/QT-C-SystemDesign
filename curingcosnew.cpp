...
#include "ui_curingcosnew.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringCosNew::CuringCosNew(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringCosNew)
{
    ui->setupUi(this);
    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("新建固化剂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("新建固化剂消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringCosNew::close);//这个牛逼
...
void CuringCosNew::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringCosNew::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringCosNew::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringCosNew::SaveBtnC()
{
    QString str;
    str = QString("insert into curingagentcosting values('%1','%2',%3,%4);")\
            .arg(ui->BathTubID->text()).arg(ui->CurAgeID->text()).arg(ui->First->text()).arg(ui->Second->text());
    emit sendData(str);
    ui->CurAgeID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
...
void CuringCosNew::on_CancleBtn_clicked()
{
    ui->CurAgeID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
