...
#include "ui_userlogsele.h"
#include <QtDebug>
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

UserLogSele::UserLogSele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::UserLogSele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("筛选用户操作信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&UserLogSele::close);//这个牛逼
    ...
    ui->ID->setFocus();
    setTabOrder(ui->ID,ui->UserName);
    setTabOrder(ui->UserName,ui->LoginTime);
    setTabOrder(ui->LoginTime,ui->FileAddre);



    ui->LoginTime->setPlaceholderText("0000-00-00 00:00");

    QRegExp date("[0-9]{4}(-[0-9]{2}){2}[ ]{1}[0-9]{2}(:[0-9]{2}){2}");

    ui->LoginTime->setValidator(new QRegExpValidator(date,this));
...
void UserLogSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void UserLogSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void UserLogSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void UserLogSele::SeleBtnC()
{
    QStringList str;
    str<<ui->ID->text()<<ui->UserName->text()<<ui->LoginTime->text()<<ui->FileAddre->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("ID = %1 ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("UserName = '%1' ").arg(str.at(1));
    if(str.at(2) != "") data<<QString("LoginTime like '%1%' ").arg(str.at(2));
    if(str.at(3) != "") data<<QString("FileAddre = '%1' ").arg(str.at(3));
    ...
    if(data.count() == 0)
    {
        splus = "select * from userlogin;";
    }else{
        splus = "select * from userlogin where "+data.at(0)+splus+";";
    }
...
void UserLogSele::on_CancleBtn_clicked()
{
    ui->ID->clear();
    ui->UserName->clear();
    ui->LoginTime->clear();
