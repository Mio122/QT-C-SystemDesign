...
#include "ui_tablebathdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

TableBathDele::TableBathDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TableBathDele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("删除浴缸信息"));//设置软件标题
    ...
    ui->label->setText(tr("删除浴缸信息"));


    ui->DeleBtn->setGeometry(370,360,75,30);
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&TableBathDele::close);//这个牛逼
    ...
    ui->BathTubName->setEnabled(false);
    ui->BathTubH->setEnabled(false);
    ui->BathTubW->setEnabled(false);
    ui->ResinID->setEnabled(false);
    ui->FiberGlassID->setEnabled(false);
    ui->CurAgID->setEnabled(false);
...
void TableBathDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void TableBathDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void TableBathDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void TableBathDele::deleData(QStringList data)
{
...
void TableBathDele::DeleBtnC()
{
    QString str;
    str = QString("delete from bathtub where BathID = '%1';").arg(ui->BathTubID->text());
    emit sendData(str);
    this->hide();

}

void TableBathDele::on_CancleBtn_clicked()
{
    this->hide();
}
