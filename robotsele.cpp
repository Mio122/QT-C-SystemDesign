#include "robotsele.h"
#include "ui_robotsele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

RobotSele::RobotSele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RobotSele)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("筛选机器人信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&RobotSele::close);//这个牛逼
...
void RobotSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void RobotSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void RobotSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void RobotSele::SeleBtnC()
{
    QStringList str;
    str<<ui->RobotID->text()<<ui->RobotName->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("RobotID = '%1' ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("RobotName = '%1' ").arg(str.at(1));
    ...
    if(data.count() == 0)
    {
        splus = "select * from robot;";
    }else{
        splus = "select * from robot where "+data.at(0)+splus+";";
    }
...
void RobotSele::on_CancleBtn_clicked()
{
    ui->RobotID->setFocus();
    ui->RobotID->clear();
    ui->RobotName->clear();
