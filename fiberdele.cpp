#include "fiberdele.h"
#include "ui_fiberdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberDele::FiberDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberDele)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("删除玻纤信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberDele::close);//这个牛逼
    ...
    ui->FiberGlassName->setEnabled(false);
    ui->FiberGlassPrice->setEnabled(false);
...
void FiberDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberDele::DeleBtnC()
{
    QString str;
    str = QString("delete from fiberglass where FiberGlassID = '%1';").arg(ui->FiberGlassID->text());
    emit sendData(str);
    this->hide();
}

void FiberDele::deleData(QStringList data)
{
...
void FiberDele::on_CancleBtn_clicked()
{
    this->hide();
}
