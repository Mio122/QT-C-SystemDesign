...
#include "ui_curingcosdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringCosDele::CuringCosDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringCosDele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("删除固化剂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("删除固化剂消耗标准信息"));


    ui->DeleBtn->setGeometry(370,240,75,30);
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringCosDele::close);//这个牛逼
...
void CuringCosDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringCosDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringCosDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringCosDele::DeleBtnC()
{
    QString str;
    str = QString("delete from curingagentcosting where BathID = '%1';").arg(ui->BathTubID->text());
    emit sendData(str);
    this->hide();

}

void CuringCosDele::deleData(QStringList data)
{
...
void CuringCosDele::on_CancleBtn_clicked()
{
    this->hide();
}
