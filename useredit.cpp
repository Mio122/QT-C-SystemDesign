...
void UserEdit::editData(QStringList data)
{
    ui->UserLineEdit->setText(data.at(0));
    str = data;
    if("管理员" == data.at(1))
    {
...
{
    if(ui->PasswordEdit->text() == str.at(2) && ui->NewPassEdit->text() != str.at(2) && ui->NewPassEdit->text() == ui->SurePassEdit->text())
    {
        QString data;
        data = QString("update users set PassWord = '%1' where UserName = '%2';").arg(ui->NewPassEdit->text()).arg(str.at(0));
        emit sendData(data);
    }
    this->hide();
