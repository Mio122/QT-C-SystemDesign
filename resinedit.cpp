#include "resinedit.h"
#include "ui_resinedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

ResinEdit::ResinEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ResinEdit)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("编辑树脂信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinEdit::close);//这个牛逼
    ...
    ui->ResinName->setFocus();
    setTabOrder(ui->ResinName,ui->ResinPrice);
...
void ResinEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinEdit::editData(QStringList data)
{
    ui->ResinID->setText(data.at(0));
    ui->ResinName->setText(data.at(1));
    ui->ResinPrice->setText(data.at(2));
    this->show();

}

void ResinEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->ResinID->text()<<ui->ResinName->text()<<ui->ResinPrice->text();
    QString data;
    data = QString("update resin set ResinName = '%1',ResinPrice = %2 where ResinID = '%3';")\
...
void ResinEdit::on_CancleBtn_clicked()
{
    this->hide();
}
