#include "resindele.h"
#include "ui_resindele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

ResinDele::ResinDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ResinDele)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("删除树脂信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinDele::close);//这个牛逼
    ...
    ui->ResinName->setEnabled(false);
    ui->ResinPrice->setEnabled(false);
...
void ResinDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinDele::DeleBtnC()
{
    QString str;
    str = QString("delete from resin where ResinID = '%1';").arg(ui->ResinID->text());
    emit sendData(str);
    this->hide();

}

void ResinDele::deleData(QStringList data)
{
...
void ResinDele::on_CancleBtn_clicked()
{
    this->hide();
}
