#include "robotedit.h"
#include "ui_robotedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

RobotEdit::RobotEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RobotEdit)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("编辑机器人信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&RobotEdit::close);//这个牛逼
...
void RobotEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void RobotEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void RobotEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void RobotEdit::editData(QStringList data)
{
...
void RobotEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->RobotID->text()<<ui->RobotName->text();
    QString data;
    data = QString("update robot set RobotName = '%1' where RobotID = '%2';")\
                    .arg(str.at(1)).arg(str.at(0));
    emit sendData(data);
    this->hide();

}

void RobotEdit::on_CancleBtn_clicked()
{
    this->hide();
}
