#include "sqlsear.h"
#include "ui_sqlsear.h"
#include <QtDebug>
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

SQLSear::SQLSear(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::SQLSear)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("高级"));//设置软件标题
    this->setWindowFlags(Qt::FramelessWindowHint|Qt::WindowStaysOnTopHint);//去掉标题栏
    this->setAttribute(Qt::WA_TranslucentBackground);//设置窗口背景透明
    this->setFixedSize(600,200);//设置窗口为固定大小

    ui->TitleLab->setGeometry(0,0,600,30);//设置标题区域位置和大小
    ...
    ui->InerLab->setGeometry(0,30,600,170);//设置标题区域大小
    ui->InerLab->setStyleSheet("QLabel{background-color:rgb(255,255,255);}");//设置标题区域颜色



    ui->label->setGeometry(10,0,80,30);
    ui->label->setText(tr("SQL高级"));


    ui->RunBtn->setGeometry(500,160,75,30);
    ui->CancleBtn->setGeometry(420,160,75,30);
    ui->textEdit->setGeometry(10,40,580,110);
    ui->CloseBtn->setGeometry(562,0,30,30);
    ui->CloseBtn->setIcon(QIcon(":/image/ICON/close2.png"));
    ui->CloseBtn->setToolTip(tr("关闭"));

    connect(ui->CloseBtn,&QPushButton::clicked,this,&SQLSear::close);//这个牛逼
    ...
    ui->RunBtn->setFocusPolicy(Qt::NoFocus);
    ...
    this->setStyleSheet("QLabel{font-family:'Microsoft YaHei';font-size:15px;}"
                        ...
                        "QTextEdit{border:1px groove gray;border-radius:0px;padding:2px 4px;"
                        ...
                        "QPushButton#RunBtn{background-color:rgb(76,136,255);}"
                        "QPushButton:pressed#RunBtn{background-color:rgb(53,95,179);}");
    //信号与槽
    connect(ui->RunBtn,SIGNAL(clicked(bool)),this,SLOT(RunBtnC()));
}
...
void SQLSear::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void SQLSear::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void SQLSear::mouseReleaseEvent(QMouseEvent *event)
{
...
void SQLSear::on_CancleBtn_clicked()
{
    ui->textEdit->clear();
    this->hide();
}
