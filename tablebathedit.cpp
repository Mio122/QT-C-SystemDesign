...
#include "ui_tablebathedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

TableBathEdit::TableBathEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TableBathEdit)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("编辑浴缸信息"));//设置软件标题
    ...
    ui->label->setText(tr("编辑浴缸信息"));


    ui->SaveBtn->setGeometry(370,360,75,30);
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&TableBathEdit::close);//这个牛逼
...
void TableBathEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void TableBathEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void TableBathEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void TableBathEdit::editData(QStringList data)
{
    ui->BathTubID->setText(data.at(0));
    ui->BathTubName->setText(data.at(1));
    ui->BathTubH->setText(data.at(2));
    ui->BathTubW->setText(data.at(3));
    ui->ResinID->setText(data.at(4));
    ui->FiberGlassID->setText(data.at(5));
    ui->CurAgID->setText(data.at(6));
    this->show();

}

void TableBathEdit::SaveBtnC()
{
    QStringList str;
    ...
    QString data;
    data = QString("update bathtub set BathTubName = '%1',BathTubH = %2,BathTubW = %3,\
                    ResinID = '%4',FiberGlassID = '%5',CurAgeID = '%6' where BathID = '%7'")\
                    .arg(str.at(1)).arg(str.at(2)).arg(str.at(3)).arg(str.at(4)).arg(str.at(5))\
                    .arg(str.at(6)).arg(str.at(0));
    emit sendData(data);
    this->hide();

}

void TableBathEdit::on_CancleBtn_clicked()
{
    this->hide();
}
