...
#include "ui_userlogdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

UserLogDele::UserLogDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::UserLogDele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("删除用户操作信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&UserLogDele::close);//这个牛逼
    ...
    ui->LoginTime->setEnabled(false);
    ui->FileAddre->setEnabled(false);
...
void UserLogDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void UserLogDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void UserLogDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void UserLogDele::DeleBtnC()
{
    QString str;
    str = QString("delete from userlogin where ID = '%1';").arg(ui->ID->text());
    emit sendData(str);
    this->hide();

}

void UserLogDele::deleData(QStringList data)
{
    ui->ID->setText(data.at(0));
    ui->UserName->setText(data.at(1));
    ui->LoginTime->setText(data.at(2));
    ui->FileAddre->setText(data.at(3));
    this->show();


}

void UserLogDele::on_CancleBtn_clicked()
{
    this->hide();
}
