...
#include "ui_resincosnew.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

ResinCosNew::ResinCosNew(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ResinCosNew)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("新建树脂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("新建树脂消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinCosNew::close);//这个牛逼
...
void ResinCosNew::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinCosNew::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinCosNew::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinCosNew::SaveBtnC()
{
    QString str;
    str = QString("insert into resincosting values('%1','%2',%3,%4);")\
            .arg(ui->BathTubID->text()).arg(ui->ResinID->text()).arg(ui->First->text()).arg(ui->Second->text());
    emit sendData(str);
    ui->ResinID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
...
void ResinCosNew::on_CancleBtn_clicked()
{
    ui->ResinID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
