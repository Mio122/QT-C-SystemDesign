#include "fibernew.h"
#include "ui_fibernew.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberNew::FiberNew(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberNew)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("新建玻纤信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberNew::close);//这个牛逼
...
void FiberNew::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberNew::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberNew::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberNew::SaveBtnC()
{
    QString str;
    str = QString("insert into fiberglass values('%1','%2',%3);")\
            .arg(ui->FiberGlassID->text()).arg(ui->FiberGlassName->text()).arg(ui->FiberGlassPrice->text());
    emit sendData(str);
...
void FiberNew::on_CancleBtn_clicked()
{
    ui->FiberGlassID->clear();
    ui->FiberGlassName->clear();
    ui->FiberGlassPrice->clear();
