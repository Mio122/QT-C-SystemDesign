...
#include "ui_resincosedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

ResinCosEdit::ResinCosEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ResinCosEdit)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("编辑树脂消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("编辑树脂消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&ResinCosEdit::close);//这个牛逼
...
void ResinCosEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void ResinCosEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void ResinCosEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void ResinCosEdit::editData(QStringList data)
{
    ui->BathTubID->setText(data.at(0));
    ui->ResinID->setText(data.at(1));
...
void ResinCosEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->BathTubID->text()<<ui->ResinID->text()<<ui->First->text()<<ui->Second->text();
    QString data;
    data = QString("update resincosting set ResinID = '%1',First = %2 ,Second = %3 where BathID = '%4';")\
...
void ResinCosEdit::on_CancleBtn_clicked()
{
    this->hide();
}
