...
#include "ui_workdataedit.h"
#include <QtDebug>
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

WorkDataEdit::WorkDataEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::WorkDataEdit)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("编辑工作记录信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&WorkDataEdit::close);//这个牛逼
...
void WorkDataEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void WorkDataEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void WorkDataEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void WorkDataEdit::editData(QStringList data)
{
...
void WorkDataEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->ID->text()<<ui->RobotID->text()<<ui->BathTubID->text()<<ui->BathMark->text()
       <<ui->SparyNum->text()<<ui->ResinCos->text()<<ui->FiberGlassCos->text()<<ui->CurAgCos->text()
       <<ui->dateEdit->text()<<ui->timeEdit->text();

    QString data;
    data = QString("update workdata set RobotID = '%1',BathID = '%2',BathMark = '%3',\
                    SparyNum = %4,ResinCos = %5,FiberGlassCos = %6,\
                    CurAgeCos = %7,WDate = '%8',WTime = '%9' where ID = %10;")\
                    .arg(str.at(1)).arg(str.at(2)).arg(str.at(3)).arg(str.at(4)).arg(str.at(5))\
                    .arg(str.at(6)).arg(str.at(7)).arg(str.at(8)).arg(str.at(9)).arg(str.at(0));
    emit sendData(data);
    qDebug()<<data;
    this->hide();
...
void WorkDataEdit::on_CancleBtn_clicked()
{
    this->hide();
}
