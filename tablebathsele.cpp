    ...
    ui(new Ui::TableBathSele)
{
    ...
    ui->label->setText(tr("筛选浴缸信息"));


    ui->SeleBtn->setGeometry(370,360,75,30);
    ui->CancleBtn->setGeometry(290,360,75,30);
    ui->CloseBtn->setGeometry(432,0,30,30);
    ui->CloseBtn->setIcon(QIcon(":/image/ICON/close2.png"));
    ui->CloseBtn->setToolTip(tr("关闭"));

    connect(ui->CloseBtn,&QPushButton::clicked,this,&TableBathSele::close);//这个牛逼
    ...
    ui->BathTubID->setFocus();

    setTabOrder(ui->BathTubID,ui->BathTubName);
    setTabOrder(ui->BathTubName,ui->BathTubH);
    setTabOrder(ui->BathTubH,ui->BathTubW);
    setTabOrder(ui->BathTubW,ui->ResinID);
    setTabOrder(ui->ResinID,ui->FiberGlassID);
    setTabOrder(ui->FiberGlassID,ui->CurAgID);
...
void TableBathSele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void TableBathSele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void TableBathSele::mouseReleaseEvent(QMouseEvent *event)
{
...
void TableBathSele::SeleBtnC()
{
    QStringList str;
    str<<ui->BathTubID->text()<<ui->BathTubName->text()<<ui->BathTubH->text()\
      <<ui->BathTubW->text()<<ui->ResinID->text()<<ui->FiberGlassID->text()<<ui->CurAgID->text();
    QStringList data;
    if(str.at(0) != "") data<<QString("BathID = '%1' ").arg(str.at(0));
    if(str.at(1) != "") data<<QString("BathTubName = '%1' ").arg(str.at(1));
    if(str.at(2) != "") data<<QString("BathTubH = %1 ").arg(str.at(2));
    if(str.at(3) != "") data<<QString("BathTubW = %1 ").arg(str.at(3));
    if(str.at(4) != "") data<<QString("ResinID = '%1' ").arg(str.at(4));
    if(str.at(5) != "") data<<QString("FiberGlassID = '%1' ").arg(str.at(5));
    if(str.at(6) != "") data<<QString("CurAgeID = '%1' ").arg(str.at(6));
    QString splus = "";
    ...
    if(data.count() == 0)
    {
        splus = "select * from bathtub;";
    }else{
        splus = "select * from bathtub where "+data.at(0)+splus+";";
    }
    ...
    ui->CurAgID->clear();
    ui->ResinID->clear();
    ui->FiberGlassID->clear();
    ui->BathTubH->clear();
    ui->BathTubW->clear();
    ui->BathTubName->clear();
    this->hide();
}

void TableBathSele::on_CancleBtn_clicked()
{
    ui->BathTubID->clear();
    ui->CurAgID->clear();
    ui->ResinID->clear();
