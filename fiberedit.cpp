#include "fiberedit.h"
#include "ui_fiberedit.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberEdit::FiberEdit(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberEdit)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("编辑玻纤信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberEdit::close);//这个牛逼
...
void FiberEdit::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberEdit::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberEdit::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberEdit::editData(QStringList data)
{
    ui->FiberGlassID->setText(data.at(0));
    ui->FiberGlassName->setText(data.at(1));
    ui->FiberGlassPrice->setText(data.at(2));
    this->show();

}

void FiberEdit::SaveBtnC()
{
    QStringList str;
    str<<ui->FiberGlassID->text()<<ui->FiberGlassName->text()<<ui->FiberGlassPrice->text();
    QString data;
    data = QString("update fiberglass set FiberGlassName = '%1',FiberGlassPrice = %2 where FiberGlassID = '%4';")\
                    .arg(str.at(1)).arg(str.at(2)).arg(str.at(0));
    emit sendData(data);
    this->hide();

}

void FiberEdit::on_CancleBtn_clicked()
{
    this->hide();
}
