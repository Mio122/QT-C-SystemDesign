#include "robotdele.h"
#include "ui_robotdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

RobotDele::RobotDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::RobotDele)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("删除机器人信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&RobotDele::close);//这个牛逼
...
void RobotDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void RobotDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void RobotDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void RobotDele::DeleBtnC()
{
    QString str;
    str = QString("delete from robot where RobotID = '%1';").arg(ui->RobotID->text());
    emit sendData(str);
    this->hide();

}

void RobotDele::deleData(QStringList data)
{
    ui->RobotID->setText(data.at(0));
    ui->RobotName->setText(data.at(1));
    this->show();

}

void RobotDele::on_CancleBtn_clicked()
{
    this->hide();
}
