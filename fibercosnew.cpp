...
#include "ui_fibercosnew.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

FiberCosNew::FiberCosNew(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::FiberCosNew)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("新建玻纤消耗标准信息"));//设置软件标题
    ...
    ui->label->setText(tr("新建玻纤消耗标准信息"));
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&FiberCosNew::close);//这个牛逼
...
void FiberCosNew::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void FiberCosNew::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void FiberCosNew::mouseReleaseEvent(QMouseEvent *event)
{
...
void FiberCosNew::SaveBtnC()
{
    QString str;
    str = QString("insert into fiberglasscosting values('%1','%2',%3,%4);")\
            .arg(ui->BathTubID->text()).arg(ui->FiberGlassID->text()).arg(ui->First->text()).arg(ui->Second->text());
    emit sendData(str);
...
void FiberCosNew::on_CancleBtn_clicked()
{
    ui->FiberGlassID->clear();
    ui->BathTubID->clear();
    ui->First->clear();
