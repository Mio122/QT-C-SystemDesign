    ...
    ui(new Ui::UserNew)
{
    ...
    ui->SurePaaEdit->setEchoMode(QLineEdit::Password);//设置输入时不显示文字，显示特殊字符
    ...
    connect(ui->ManRBtn,SIGNAL(toggled(bool)),this,SLOT(setType()));
    connect(ui->NorRBtn,SIGNAL(toggled(bool)),this,SLOT(setTypeT()));
...
void UserNew::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void UserNew::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void UserNew::mouseReleaseEvent(QMouseEvent *event)
{
...
void UserNew::SaveBtnC()
{
    if(ui->PasswordEdit->text() == ui->SurePaaEdit->text())
    {
        QString str = QString("insert into users values('%1','%2','%3');").arg(ui->UserEdit->text()).arg(type).arg(ui->PasswordEdit->text());
        qDebug()<<str;
        emit sendData(str);
        ui->UserEdit->clear();
        ui->SurePaaEdit->clear();
...
void UserNew::setType()
{
    type = "管理员";
}

void UserNew::setTypeT()
{
    type = "普通用户";
}

void UserNew::CloseBtnC()
{
    ui->UserEdit->clear();
    ui->SurePaaEdit->clear();
    ui->PasswordEdit->clear();
...
void UserNew::on_CancleBtn_clicked()
{
    ui->UserEdit->clear();
    ui->SurePaaEdit->clear();
    ui->PasswordEdit->clear();
