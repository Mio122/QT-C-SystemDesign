...
#include "ui_curingdele.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

CuringDele::CuringDele(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CuringDele)
{
    ui->setupUi(this);

    setWindowIcon(QIcon(":/image/ICON/engineer.png"));//设置软件图标
    setWindowTitle(tr("删除固化剂信息"));//设置软件标题
    ...
    ui->label->setText(tr("删除固化剂信息"));


    ui->DeleBtn->setGeometry(370,200,75,30);
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&CuringDele::close);//这个牛逼
    ...
    ui->CurAgeID->setEnabled(false);
    ui->CurAgeName->setEnabled(false);
    ui->CurAgePrice->setEnabled(false);
...
void CuringDele::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void CuringDele::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void CuringDele::mouseReleaseEvent(QMouseEvent *event)
{
...
void CuringDele::DeleBtnC()
{
    QString str;
    str = QString("delete from curingagent where CurAgeID = '%1';").arg(ui->CurAgeID->text());
    emit sendData(str);
    this->hide();

}

void CuringDele::deleData(QStringList data)
{
    ui->CurAgeID->setText(data.at(0));
    ui->CurAgeName->setText(data.at(1));
    ui->CurAgePrice->setText(data.at(2));
    this->show();

}

void CuringDele::on_CancleBtn_clicked()
{
    this->hide();
}
