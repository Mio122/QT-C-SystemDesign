#include "tablebath.h"
#include "ui_tablebath.h"
#include <QMouseEvent>//用到QMouseEvent就要包含这个头文件

TableBath::TableBath(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::TableBath)
{
    ui->setupUi(this);
    ...
    setWindowTitle(tr("新建浴缸信息"));//设置软件标题
    ...
    connect(ui->CloseBtn,&QPushButton::clicked,this,&TableBath::close);//这个牛逼
...
void TableBath::mousePressEvent(QMouseEvent *event)
{
...
//···鼠标移动操作···
//实时修改窗口的坐标
void TableBath::mouseMoveEvent(QMouseEvent *event)
{
...
//···鼠标释放操作···
void TableBath::mouseReleaseEvent(QMouseEvent *event)
{
...
void TableBath::SaveBtnC()
{
    QString str;
    str = QString("insert into bathtub values('%1','%2',%3,%4,'%5','%6','%7');")\
            .arg(ui->BathTubID->text()).arg(ui->BathTubName->text()).arg(ui->BathTubH->text())\
            .arg(ui->BathTubW->text()).arg(ui->ResinID->text()).arg(ui->FiberGlassID->text())\
            .arg(ui->CurAgID->text());
    emit sendData(str);
    ui->BathTubID->clear();
    ui->CurAgID->clear();
...
void TableBath::on_CancleBtn_clicked()
{
    ui->BathTubID->clear();
    ui->CurAgID->clear();
    ui->ResinID->clear();
